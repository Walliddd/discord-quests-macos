/*
 * Discord Quests Completer — Windows build
 * ----------------------------------------
 * Standalone Windows-only version. If you're unsure which file to use, use the
 * universal ../quest-completer.js instead (it auto-detects your OS).
 *
 * Game quests spoof a `win32` process at C:\Program Files\<App>\<App>.exe
 *
...
(() => {
  ...
  const buildFakeGame = (appData, applicationId, pid) => {
    ...
    // Discord's win32 executable names are sometimes prefixed with ">" (launcher flag).
    const exeName = appData.executables?.find(x => x.os === "win32")?.name?.replace(/^>/, "") ?? `${name}.exe`;
    return {
  ...
  const doJob = () => {
    ...
    if (taskName === "WATCH_VIDEO" || taskName === "WATCH_VIDEO_ON_MOBILE") {
      ...
      api.get({ url: `/applications/public?application_ids=${applicationId}` }).then(res => {
        const fakeGame = buildFakeGame(res.body[0], applicationId, pid);
